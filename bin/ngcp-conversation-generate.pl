...
my $SUBSCRIBER_TEMPLATE = {
    ...
    #we repeate it intentionally, to simplify logic
    ...
    && scalar @{$SUBSCRIBERS_EXISTING_ID->{callee}} >= $AMOUNT->{SUBSCRIBERS}) )
{
    ...
    if($contact->{reseller_id} != $domain->{reseller_id}){
        die("Precondition not met: Domain ".$domain->{domain}." should belong to the reseller_id = ".$contact->{reseller_id}.", but it belongs to ".$domain->{reseller_id}.".", 1);
    }
    if('pbxaccount' eq $customer->{type}){
        $remote_config = $api_client->request('GET','/api/metaconfigdefs/')->as_hash;
        if($remote_config->{config}->{features}->{cloudpbx}){
            my $get_pilot_res = $api_client->request('GET','/api/subscribers/?customer_id='.$customer->{id}.'&is_pbx_pilot=1');
            if(!$get_pilot_res->get_total_count){
                print "Create pilot automatically?\n";
                chomp (my $choice =<STDIN>);
                if($choice =~/y|yes/i){
                    $create_pilot = 1;
                }else{
                    die('You will need to create pilot manually.');
                }
            }
        }else{
            die('Misconfigured customer. Customer type is '.$customer->{type}.' but cloudpbx feature is off in ngcp-config!');
        }
    }
}
...
{#prepare subscribers
    ...
    foreach my $type (qw/caller callee/){
        ...
        for(my $i = 1; $i <= $AMOUNT->{SUBSCRIBERS}; $i++){
                ...
                if($get_subscriber_res->get_total_count()){
                    ...
                    my $sub_create = sub {
                        ...
                        foreach my $field(qw/username password display_name/){
                            my $format = $SUBSCRIBER_TEMPLATE->{$field.'_format'.$type_suffix} // $SUBSCRIBER_TEMPLATE->{$field.'_format'};
                            $data->{$field} = sprintf( $format, $i );
                        }
                        if('pilot' eq $type_suffix){
                            $data->{is_pbx_pilot} = 1;
                        }
                        $data->{pbx_extension} .= $type_i.$i;
                        return $api_client->request_ex('POST', '/api/subscribers/', $data)->as_hash;
                    };
                    if($create_pilot){
                        $sub_create->(0,0,'pilot');
                        $create_pilot = 0;
                    }
                    $subscriber = $sub_create->($i,$type_i,$type_suffix);
                }
            }
            ...
            #$faxserversettings->{password} = 'aaabbb111';
            #$faxserversettings->{name} = 'nameaaabbb111';
            #$faxserversettings->{destinations} = [{
            #    destination => 'sipwise@sp1',# !!! Form forbids it!
            #    filetype    => 'PDF',
            #}];
            my $response = $api_client->request('PUT','/api/faxserversettings/'.$subscriber->{id}, $faxserversettings);
    ...
    for (my $i_item=0; $i_item < $AMOUNT->{SUBSCRIBERS}; $i_item++){
        ...
        if($AMOUNT->{FAX}){
            $api_client->request('PATCH','/api/callforwards/'.$callee->{id},[ {
                ...
                'value' => {
                    'destinations' => [
                        { 'destination' => 'Fax2Mail', 'timeout' => 20},
                    ],
                    'times' => undef,
                },
            } ] );
