...

{
    ...
    if(!$get_customer_res->as_hash->{id}){
    ...
    my $get_domain_struct = $get_domain_res->result_struct('domains');
    if(!$get_domain_struct->get_total_count){
        die("Precondition not met: Domain not found", 1);
    }
    my $contact = $get_contact_res->as_hash;
    my $domain = $get_domain_struct->get_embedded_item(0);
    if($contact->{reseller_id} != $domain->{reseller_id}){
        die("Precondition not met: Domain should belong to the reseller_id = ".$customer->{content}->{reseller_id}, 1);
    }
}
{#prepare subscribers
    my $type_i = 1;
    foreach my $type (qw/caller callee/){
        ...
        for(my $i = 1; $i <= $AMOUNT->{SUBSCRIBERS}; $i++){
            ...
            my $get_subscriber_struct = $get_subscriber_res->result_struct('subscribers');
            if(!$get_subscriber_struct->get_total_count()){
                ...
                my $post_subscriber_res = $api_client->request('POST','/api/subscribers/', $data);
                my $post_subscriber_struct = $post_subscriber_res->result_struct('subscribers');
                $get_subscriber_res = $api_client->request('GET',$post_subscriber_struct->get_created_location);
                $get_subscriber_struct = $get_subscriber_res->result_struct('subscribers');
            }
            $subscriber = $get_subscriber_struct->get_embedded_item(0);
        ...
        if($AMOUNT->{FAX}){
            foreach my $type_l (qw/caller callee/){
                my $get_faxserversettings_res = $api_client->request('GET',
                    '/api/faxserversettings/'.$subscribers->{$type_l}->[$i_item]->{id});
                my $faxserversettings = $get_faxserversettings_res->as_hash;
                $faxserversettings->{active} = 1;
                $faxserversettings->{password} = 'aaa111';
                $api_client->request('PUT','/api/faxserversettings/'.$subscribers->{$type_l}->[$i_item]->{id}, $faxserversettings);    
            }
