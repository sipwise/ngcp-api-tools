...
#apt-get install libnet-address-ip-local-perl
#apt-get install libnet-sip-perl
#apt-get install libanyevent-xmpp-perl
# ===>
# apt-get install libnet-ifconfig-wrapper-perl; apt-get install libnet-address-ip-local-perl; apt-get install libnet-sip-perl; apt-get install libanyevent-xmpp-perl
#
...
use NetAddr::IP;
use Net::SIP::Simple; #libnet-sip-perl
use Net::SIP::Simple::Call;

use AnyEvent;#libanyevent-perl
use AnyEvent::XMPP::Client;#libanyevent-xmpp-perl
...
#$api_client->set_opts({ get_post => 1 });

$ITEM_AMOUNT //= 1;
$PORT //= 5060;
$REGS->{callee}->{domain} //= $REGS->{caller}->{domain};
$FILE //= '';
('voicemail' eq $TYPE) # || 'call' eq $TYPE
    and ($FILE ||= dirname(abs_path($0)).'/resources/test.wav');
...
if( 'call' eq $TYPE || 'voicemail' eq $TYPE ){
    register_ips();
    $thread_uas = threads->create(\&sip_uas, $ITEM_SERVER, $PORT,
        @{$REGS->{callee}}{qw/ip domain username password/}, $FILE
    );
    $thread_uac = threads->create(\&sip_uac, $ITEM_AMOUNT, $ITEM_SERVER, $PORT,
        @{$REGS->{caller}}{qw/ip domain username password/},
        join('@', @{$REGS->{callee}}{qw/username domain/}), $FILE
    );
}elsif( 'xmpp' eq $TYPE ){
    $thread_uac = threads->create(\&xmpp, $ITEM_AMOUNT, $ITEM_SERVER, $PORT,
        @{$REGS->{caller}}{qw/domain username password/},
        @{$REGS->{callee}}{qw/domain username password/},
    );
}elsif( 'sms' eq $TYPE ){
    $thread_uac = threads->create(\&sms, $ITEM_AMOUNT,
        $REGS->{caller}->{subscriber_id},
        join('',@{$REGS->{callee}->{item}->{primary_number}}{qw/ac cc sn/})
    );
}elsif( 'fax' eq $TYPE ){
    $thread_uac = threads->create(\&fax, $ITEM_AMOUNT,
        $REGS->{caller}->{subscriber_id},
        join('',@{$REGS->{callee}->{item}->{primary_number}}{qw/ac cc sn/})
    );
}
...
sub get_items_amount{
    foreach my $type(qw/caller callee/){
        ...
        if(!$REGS->{$type}->{subscriber_id} ){
            ...
            $REGS->{$type}->{item} = $res->as_hash()->{_embedded}->{'ngcp:subscribers'}->[0];
        }
        ...
        $REGS->{$type}->{items_amount} = $res->as_hash()->{total_count};
        $REGS->{$type}->{items_amount_initial} //= $REGS->{$type}->{items_amount};
        print Dumper [$REGS->{$type}->{items_amount},$REGS->{$type}->{items_amount_initial}] ;
    }
}

sub fax{
    my($amount,$caller_subscriber_id,$callee_cli) = @_;
    local $SIG{'KILL'} = sub { threads->exit(); };

    my $faxfile = Path::Tiny->tempfile;
    write_file( $faxfile->stringify, {binmode => ':raw'}, ("Test message created at ".time()."\n") x 5);
    my $data =  {
        json => {
            subscriber_id => "$caller_subscriber_id",
            destination => "$callee_cli",
        },
        faxfile => [ $faxfile->stringify ],
    };

    for(my $i=0; $i < $amount; $i++){
        my $res = $api_client->request('POST','/api/faxes/', $data);
    }
}
...
sub sms{
    ...
    my $data = {
        subscriber_id => "$caller_subscriber_id",
        text => "Test message created at ".time(),
        callee => "$callee_cli",
        direction => 'out',
    };
    print Dumper $data;
    for(my $i=0; $i < $amount; $i++){
        #curl -k -X POST --user xxx:xxx -H 'Connection: close' -H 'Content-Type: application/json'
        #'https://127.0.0.1:1443/api/sms/?test_mode=dont_send_sms'
        #--data-binary '{ "subscriber_id" : "365", "text" : "aaa", "callee" : "11151327432412","direction" :"out" }'
        my $res = $api_client->request('POST','/api/sms/?test_mode=dont_send_sms',$data);
    }
}
...
sub sip_uas{
    ...
    my %listen_params = (
        cb_cleanup => sub { print "uas: call_ended;\n"; $call_ended = 1; },
    ...
    for(my $i=0; $i < $items_amount; $i++){
        ...
        my %call_params = (
            $file ? (
                ...
                'cb_rtp_done' => sub { print "uac: rtp_done;\n"; $rtp_done = 1; },
                'asymetric_rtp' => 0,
                'rtp_param' => [8, 160, 160/8000, 'PCMA/8000'],
            ) : (
                'cb_final' => sub {$call_done = 1 ; print "uac: cb_final;\n";},
            )
        );
        if($callee !~/^sip:/){
            $callee = 'sip:'.$callee;
        ...
        if($file){
            ...
            $call->bye( 'cb_final' => sub {$call_done = 1 ; print "uac: cb_final from bye;\n";} );
        }else{
            $call->loop( \$call_done, 10 );
            $call->bye;
       }
    }
}
